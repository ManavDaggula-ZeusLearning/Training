...
CREATE STREAM DISTRICT_KEY_VALUES (ROWKEY STRUCT<ID INT> KEY, BEFORE STRUCT<ID INT, NAME VARCHAR>, AFTER STRUCT<ID INT, NAME VARCHAR>, OP VARCHAR, TS_MS BIGINT) WITH (KAFKA_TOPIC='dbserver1.kafkadb.district', VALUE_FORMAT='JSON', KEY_FORMAT='JSON', PARTITIONS=5);
CREATE STREAM SCHOOLS_KEY_VALUES(ROWKEY STRUCT<ID INT> KEY, BEFORE STRUCT<ID INT, NAME VARCHAR, DISRTICT_ID INT>, AFTER STRUCT<ID INT, NAME VARCHAR, DISTRICT_ID INT>, OP VARCHAR, TS_MS BIGINT) WITH (KAFKA_TOPIC='dbserver1.kafkadb.schools', VALUE_FORMAT='JSON', KEY_FORMAT='JSON', PARTITIONS=5);
CREATE STREAM CLASS_KEY_VALUES(ROWKEY STRUCT<ID INT> KEY, BEFORE STRUCT<ID INT, NAME VARCHAR, SCHOOL_ID INT>, AFTER STRUCT<ID INT, NAME VARCHAR, SCHOOL_ID INT>, OP VARCHAR, TS_MS BIGINT) WITH (KAFKA_TOPIC='dbserver1.kafkadb.class', VALUE_FORMAT='JSON', KEY_FORMAT='JSON', PARTITIONS=5);
CREATE STREAM USER_KEY_VALUES(ROWKEY STRUCT<ID INT> KEY, BEFORE STRUCT<ID INT, NAME VARCHAR, DOB DATE>, AFTER STRUCT<ID INT, NAME VARCHAR, DOB DATE>, OP VARCHAR, TS_MS BIGINT) WITH (KAFKA_TOPIC='dbserver1.kafkadb.users', VALUE_FORMAT='JSON', KEY_FORMAT='JSON', PARTITIONS=5);
CREATE STREAM ENROLLMENT_KEY_VALUES(ROWKEY STRUCT<ID INT> KEY, BEFORE STRUCT<USER_ID INT, CLASS_ID INT, ID INT>, AFTER STRUCT<USER_ID INT, CLASS_ID INT, ID INT>, OP VARCHAR, TS_MS BIGINT) WITH (KAFKA_TOPIC='dbserver1.kafkadb.enrollment', KEY_FORMAT='JSON',VALUE_FORMAT='JSON', PARTITIONS=5);
CREATE STREAM LOGIN_KEY_VALUES(AFTER STRUCT<USER_ID INT, TIMESTAMP TIMESTAMP>) WITH (KAFKA_TOPIC='dbserver1.kafkadb.login', VALUE_FORMAT='JSON', PARTITIONS=5);
